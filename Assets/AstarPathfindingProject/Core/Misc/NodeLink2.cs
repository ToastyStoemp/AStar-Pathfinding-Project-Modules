...
namespace Pathfinding {
	...
	public class NodeLink2 : GraphModifier {
		...
		public void Apply (bool forceNewCheck) {
			...
			if (!oneWay) connectedNode2.AddConnection(endNode, (uint)Mathf.RoundToInt(((Int3)(clamped2 - EndTransform.position)).costMagnitude*costFactor), (uint)setTag);

			if (!oneWay) startNode.AddConnection(connectedNode1, (uint)Mathf.RoundToInt(((Int3)(clamped1 - StartTransform.position)).costMagnitude*costFactor), (uint)setTag);
